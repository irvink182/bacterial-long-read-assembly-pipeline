...
def build_matrix(df, sample_col, gene_col, all_samples):
    if df.empty:
        return pd.DataFrame({"sample": all_samples})
...
def main():
    ...
    all_samples = sorted(df[sample_col].dropna().unique())
    ...
    for t in type_list:
        ...
        if df_t.empty:
            print(f"[INFO] No genes for type {t}, creating empty matrix")
            out = pd.DataFrame({"sample": all_samples})
        else:
            ...
            out = build_matrix(df_t, sample_col, gene_col, all_samples)
